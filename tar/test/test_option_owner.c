...
DEFINE_TEST(test_option_owner)
{
	...
	failure("Error invoking %s -c", testprog);
	assertEqualInt(0,
	    systemf("%s -c -f archive2 --owner=65123 --format=ustar file >stdout2.txt 2>stderr2.txt",
		testprog));
	...
	failure("Error invoking %s -c", testprog);
	assertEqualInt(0,
	    systemf("%s -c -f archive3 --owner=foofoofoo --format=ustar file >stdout3.txt 2>stderr3.txt",
		testprog));
	...
	failure("Error invoking %s -c", testprog);
	assertEqualInt(0,
	    systemf("%s -c -f archive4 --owner=foofoofoo:65123 --format=ustar file >stdout4.txt 2>stderr4.txt",
		testprog));
